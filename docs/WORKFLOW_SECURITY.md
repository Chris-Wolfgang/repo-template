...
- `.github/license-audit/*.json` - License-audit policy (allow-list, ignored packages, URL mappings, overrides). `license-audit.yaml` takes these from `main` itself, so they are not overwritten by `pr.yaml`, but the guard classifies a change to them as configuration

All wildcard entries (`*.editorconfig`, `*.globalconfig`, `*.ruleset`, `*.DotSettings`, `.github/workflows/*.yml`, `.github/workflows/*.yaml`) are matched case-insensitively — Windows (and ReSharper on it) resolve `foo.dotsettings` and `foo.DotSettings` to the same file, so both are protected — and `*.editorconfig` covers the root file and every nested one (a per-project `.editorconfig` overrides analyzer severities for everything beneath it). The fixed names (`Directory.Build.props`, `Directory.Build.targets`, `BannedSymbols.txt`, `coverlet.runsettings`, `.config/dotnet-tools.json`) are matched exactly. A protected file the PR *adds* has no `main` copy to overwrite it with, so it is removed from the checkout before the build.
